...
from src.ghstars.net.http import MAX_RETRIES, RateLimiter
...
class GitHubRepositorySearchClient:
    def __init__(
        ...
        *,
        ...
        self.rate_limiter = RateLimiter(resolve_github_search_min_interval(github_token, min_interval))
    ...
    async def search_by_paper_title(self, title: str) -> list[GitHubSearchCandidate]:
        rows = await self.collect_repositories(
            SearchRequest(query=f'"{title}"', sort="stars", order="desc")
        )
        candidates: list[GitHubSearchCandidate] = []
        seen: set[str] = set()
        for row in rows:
            normalized = normalize_github_url(row.github)
...
def split_star_range(partition: SearchPartition) -> tuple[SearchPartition, SearchPartition] | None:
...
def split_created_range(partition: SearchPartition) -> tuple[SearchPartition, SearchPartition] | None:
...
def resolve_github_search_min_interval(github_token: str, requested_min_interval: float) -> float:
    ...
    *,
    ...
    while pending:
        ...
        if split_choice is not None:
            _split_kind, children = split_choice
