...
- `docs/design`存放项目设计文档快照，里面的文件夹格式为`年-月-日-时-分-秒`，表示在当前这个时间点下的项目设计文档。因此你应该优先以新时间点的快照里面的项目设计文档为准。注意：`docs/design`除非用户明确要求，否则默认你只能查阅，不能增删改。
- `docs/plan`存放项目代码实现计划。里面的文件命名格式为`年-月-日-时-分-秒.md`。举例：`2026-04-23-13-42-43.md`。默认允许你增删改查。
