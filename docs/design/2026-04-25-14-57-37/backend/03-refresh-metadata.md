...
- `GITHUB_TOKEN`
- `REFRESH_METADATA_GITHUB_MIN_INTERVAL`，默认值为 `0.2` 秒
- `REFRESH_METADATA_GITHUB_GRAPHQL_BATCH_SIZE`
- `REFRESH_METADATA_GITHUB_REST_FALLBACK_MAX_CONCURRENT`
...
- 无 Token 时，REST 的有效最小间隔不会低于 `REFRESH_METADATA_GITHUB_ANONYMOUS_REST_MIN_INTERVAL_SECONDS`

这是为了降低匿名访问被平台限流的概率。

## 10. 当前写死但需要让用户知道的常量

### 10.1 `REFRESH_METADATA_GITHUB_GRAPHQL_MAX_CONCURRENT = 1`

当前实现里，GitHub GraphQL 批量请求在单个任务内部是串行发出的，一次只会有 `1` 个 batch 在飞。

这意味着：

- 即使 `REFRESH_METADATA_GITHUB_GRAPHQL_BATCH_SIZE` 调大，GraphQL 侧也不是多批并发
- 这是一条当前写死的吞吐上限

### 10.2 `REFRESH_METADATA_GITHUB_GRAPHQL_TOPICS_FIRST = 20`
...
### 10.3 `REFRESH_METADATA_GITHUB_ANONYMOUS_REST_MIN_INTERVAL_SECONDS = 60.0`

如果没有 `GITHUB_TOKEN`，系统会把 REST 刷新的有效最小间隔强制抬到至少 `60` 秒。

这意味着：

- 即使用户把 `REFRESH_METADATA_GITHUB_MIN_INTERVAL` 配得更小，匿名模式也不会按更小值运行
- 这个保护下限是写死的，不是配置项
